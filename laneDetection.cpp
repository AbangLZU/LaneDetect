...
void laneDetection::laneDetctAlgo()
{
    ...
    cvtColor(warpOriImage, wrapImageGray, cv::COLOR_BGR2GRAY);

    Canny(wrapImageGray, warpEdgeImage, 100, 150, 3);
    inRange(warpEdgeImage, cv::Scalar(1),cv::Scalar(255),warpEdgeImage);
...
void laneDetection::calHist()
{
    ...
    {
        cv::Mat ROI = mergeImage(cv::Rect(i, oriImage.size().height-midHeight-1, 1, midHeight));
        cv::Mat dst;
    ...
    histImage = cv::Scalar(255,255,255);

    //To create the histogram image
    for(int i=0; i<histogram.size(); i++)
    {
        line(histImage, cv::Point2f(i,(maxValue-histogram.at(i))), cv::Point2f(i,maxValue), cv::Scalar(0,0,255), 1);
    }
}
...
void laneDetection::boundaryDetection()
{
    ...
    {
        line(mergeImageRGB, cv::Point2f(leftLanePos, 0), cv::Point2f(leftLanePos, mergeImageRGB.size().height), cv::Scalar(0, 255, 0), 10);
        line(mergeImageRGB, cv::Point2f(rightLanePos, 0), cv::Point2f(rightLanePos, mergeImageRGB.size().height), cv::Scalar(0, 255, 0), 10);
    }
}


//Fitting the lane curve.
void laneDetection::laneSearch(const int &lanePos, vector<cv::Point2f> &_line, int &lanecount, vector<cv::Point2f> &curvePoints, char dir)
{
    _line.clear();
    ...
    else //Conduct search based on previous results.
    {
        ...
        {
            if(dir == 'L') line(mergeImageRGB, cv::Point2f(leftLanePos, 0), cv::Point2f(leftLanePos, mergeImageRGB.size().height), cv::Scalar(0, 255, 0), 10);

            else line(mergeImageRGB, cv::Point2f(rightLanePos, 0), cv::Point2f(rightLanePos, mergeImageRGB.size().height), cv::Scalar(0, 255, 0), 10);
        }
    }
...
cv::Mat laneDetection::getRedChannel()
{
    return imageChannels[2];
}

cv::Mat laneDetection::getRedBinary()
{
    return RedBinary;
}
