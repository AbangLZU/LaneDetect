...
laneDetection::laneDetection(const cv::Mat _oriImage, const cv::Mat _perspectiveMatrix)
        : oriImage(_oriImage), perspectiveMatrix(_perspectiveMatrix), blockNum(9), windowSize(150), recordCounter(0),
          initRecordCount(0), failDetectFlag(true) {
    ...
    if ((initRecordCount < 5) || failDetectFlag) //Conduct full search.
    {
        for (int i = 0; i < blockNum; i++) {
            ...
            if (xRB - xLU > 0 && xRB >= 0 && xLU >= 0) {
                ...
                xRB = ((xLU + windowSize) < (mergeImage.size().width)) ? (xLU + windowSize) : (mergeImage.size().width -
                                                                                               1);
                if (xRB - xLU > 0 && xRB >= 0 && xLU >= 0) {
                    for (int j = yLU; j <= yRB; j += skipStep) {
                        matPtr = mergeImage.data + (j * mergeImage.size().width);
